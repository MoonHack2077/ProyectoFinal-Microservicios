...
def read_ingredient_inventory(ingredient_inventory_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail="Ingredient inventory not found"
        ) from exc
...
def update_ingredient_inventory(
    ...
    """
    try:
        return update_ingredient_inventory_service(
            ingredient_inventory_id, ingredient_inventory_data
        )
    except DoesNotExist as exc:
        raise HTTPException(
            status_code=404, detail="Ingredient inventory not found"
